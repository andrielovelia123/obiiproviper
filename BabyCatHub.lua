...
Server:AddSection("Misc")

Server:AddToggle("Bright", {
...
spawn(function()
    while wait(0.2) do
        if QuestYama_1 == true then
            aT:SetDesc(' Quest Numbers : yama quest 1')
        elseif QuestYama_2 == true then
            aT:SetDesc(' Quest Numbers : yama quest 2')
        elseif QuestYama_3 == true then
            aT:SetDesc(' Quest Numbers : yama quest 3')
        elseif QuestTushita_1 == true then
            aT:SetDesc(' Quest Numbers : tushita quest 1')
        elseif QuestTushita_2 == true then
            aT:SetDesc(' Quest Numbers : tushita quest 2')
        elseif QuestTushita_1 == true then
            aT:SetDesc(' Quest Numbers: tushita quest 2')
        elseif GetWP('Cursed Dual Katana') then
            aT:SetDesc(' Quest Numbers: CDK done!!')
        end
    end
end)
    ...
    while wait(Sec) do
        pcall(function()
            if _G.CDK then
                ...
                local aV = GetConnectionEnemies('Cursed Skeleton Boss')
                if aV then
                    repeat
                        wait()
                        ...
                        _tp(aV.HumanoidRootPart.CFrame * CFrame.new(0, 20, 0))
                    until not _G.CDK or not aV.Parent or aV.Humanoid.Health <= 0
                else
                ...
                if tostring(replicated.Remotes.CommF_:InvokeServer('CDKQuest', 'OpenDoor')) ~= 'opened' then
                    ...
                    elseif replicated.Remotes.CommF_:InvokeServer('CDKQuest', 'Progress').Finished == false then
                        if tonumber(replicated.Remotes.CommF_:InvokeServer('CDKQuest', 'Progress').Evil) == - 3 then
                            ...
                            repeat
                                ...
                                if not workspace.Enemies:FindFirstChild('Forest Pirate') then
                                    ...
                                    local aW = GetConnectionEnemies('Forest Pirate')
                                    if aW then
                        ...
                        elseif tonumber(replicated.Remotes.CommF_:InvokeServer('CDKQuest', 'Progress').Evil) == - 4 then
                            ...
                            QuestYama_2 = true
                            QuestYama_3 = false
                            for aW, aX in pairs(game:GetService('Players').LocalPlayer.QuestHaze:GetChildren()) do
                                for aY, aZ in pairs(l) do
                                    if string.find(aY, aX.Name) and aX.Value > 0 then
                                        if (aZ.Position - Root.Position).Magnitude <= 1000 and workspace.Enemies:FindFirstChild(aY) then
                                            for a_, a0 in pairs(workspace.Enemies:GetChildren()) do
                                                if a0:FindFirstChild('HumanoidRootPart') and a0:FindFirstChild('Humanoid') and a0:FindFirstChild('Humanoid').Health > 0 and a0:FindFirstChild('HazeESP') then
                                                    repeat
                                                        wait()
                                                        m.Kill(a0, _G.CDK_YM)
                                                    until not _G.CDK_YM or tonumber(replicated.Remotes.CommF_:InvokeServer('CDKQuest', 'Progress').Evil) == 2 or not a0:FindFirstChild('HazeESP') or a0.Humanoid.Health <= 0
                                                end
                                            end
                                        else
                                            _tp(aZ)
                        ...
                        elseif tonumber(replicated.Remotes.CommF_:InvokeServer('CDKQuest', 'Progress').Evil) == - 5 then
                            ...
                            if workspace.Map:FindFirstChild('HellDimension') then
                                if (Root.Position - workspace.Map.HellDimension.Spawn.Position).Magnitude <= 1000 then
                                    for aW, aX in pairs(workspace.Map.HellDimension.Exit:GetChildren()) do
                                        if tonumber(aW) == 2 then
                                            repeat
                                                task.wait()
                                    ...
                                    if tonumber(replicated.Remotes.CommF_:InvokeServer('CDKQuest', 'Progress').Evil) ~= 3 then
                                        repeat
                                            ...
                                            repeat
                                                ...
                                                for aW, aX in pairs(workspace.Map.HellDimension:GetDescendants()) do
                                                    if aX:IsA('ProximityPrompt') then
                                    ...
                                    for aW, aX in pairs(workspace.Enemies:GetChildren()) do
                                        if (aX:FindFirstChild('HumanoidRootPart').Position - workspace.Map.HellDimension.Spawn.Position).Magnitude <= 300 then
                                            if aX:FindFirstChild('HumanoidRootPart') and aX:FindFirstChild('Humanoid') and aX:FindFirstChild('Humanoid').Health > 0 then
                                                repeat
                                                    task.wait()
                                                    m.Kill(aX, _G.CDK_YM)
                                                until not _G.CDK_YM or aX.Humanoid.Health <= 0 or not aX.Parent or tonumber(replicated.Remotes.CommF_:InvokeServer('CDKQuest', 'Progress').Evil) == 3
                                            end
                                        end
                                    end
                                end
            ...
            if _G.CDK_YM then
                if tonumber(replicated.Remotes.CommF_:InvokeServer('CDKQuest', 'Progress').Evil) == - 5 then
                    if not workspace.Map:FindFirstChild('HellDimension') or (Root.Position - workspace.Map.HellDimension.Spawn.Position).Magnitude > 1000 then
                        local aW = GetConnectionEnemies('Soul Reaper')
                        if aW then
                            repeat
                                task.wait()
                                _tp(aW.HumanoidRootPart.CFrame)
                            until aW.Humanoid.Health <= 0 or not _G.CDK_YM or not aW.Parent or tonumber(replicated.Remotes.CommF_:InvokeServer('CDKQuest', 'Progress').Evil) == 3 or (workspace.Map:FindFirstChild('HellDimension') and (Root.Position - workspace.Map.HellDimension.Spawn.Position).Magnitude <= 1000)
                        ...
                        elseif replicated:FindFirstChild('Soul Reaper') and replicated:FindFirstChild('Soul Reaper').Humanoid.Health > 0 then
                            ...
                            if replicated.Remotes.CommF_:InvokeServer('Bones', 'Check') < 50 and not workspace.Enemies:FindFirstChild('Soul Reaper') and not replicated:FindFirstChild('Soul Reaper') and not workspace.Map:FindFirstChild('HellDimension') then
                                if workspace.Enemies:FindFirstChild('Reborn Skeleton') or workspace.Enemies:FindFirstChild('Living Zombie') or workspace.Enemies:FindFirstChild('Domenic Soul') or workspace.Enemies:FindFirstChild('Posessed Mummy') then
                                    for aX, aY in pairs(workspace.Enemies:GetChildren()) do
                                        if aY.Name == 'Reborn Skeleton' or aY.Name == 'Living Zombie' or aY.Name == 'Demonic Soul' or aY.Name == 'Posessed Mummy' then
                                            if aY:FindFirstChild('HumanoidRootPart') and aY:FindFirstChild('Humanoid') and aY:FindFirstChild('Humanoid').Health > 0 then
                                                repeat
                                                    task.wait()
                                                    m.Kill(aY, _G.CDK_YM)
                                                until not _G.CDK_YM or aY.Humanoid.Health <= 0 or not aY.Parent
                                            end
                                        end
                                    end
                                else
                    ...
                    elseif replicated.Remotes.CommF_:InvokeServer('CDKQuest', 'Progress').Finished == false then
                        ...
                        elseif tonumber(replicated.Remotes.CommF_:InvokeServer('CDKQuest', 'Progress').Good) == - 5 then
                            ...
                            if workspace.Enemies:FindFirstChild('Cake Queen') then
                                for aX, aY in pairs(workspace.Enemies:GetChildren()) do
                                    if aY.Name == 'Cake Queen' then
                                        if aY:FindFirstChild('Humanoid') and aY:FindFirstChild('HumanoidRootPart') and aY.Humanoid.Health > 0 then
                                            repeat
                                                wait()
                                                m.Kill(aY, _G.CDK_TS)
                                            until not _G.CDK_TS or not aY.Parent or aY.Humanoid.Health <= 0 or tonumber(replicated.Remotes.CommF_:InvokeServer('CDKQuest', 'Progress').Good) == 3
                                        end
                                    end
                                end
                            elseif replicated:FindFirstChild('Cake Queen') and replicated:FindFirstChild('Cake Queen').Humanoid.Health > 0 then
                                ...
                                if (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - workspace.Map.HeavenlyDimension.Spawn.Position).Magnitude <= 1000 then
                                    for aX, aY in pairs(workspace.Map.HeavenlyDimension.Exit:GetChildren()) do
                                        Ex = aX
                                    end
                                    if Ex == 2 then
                                        repeat
                                    ...
                                    repeat
                                        ...
                                        repeat
                                            ...
                                            for aX, aY in pairs(workspace.Map.HeavenlyDimension:GetDescendants()) do
                                                if aY:IsA('ProximityPrompt') then
                                                    fireproximityprompt(aY)
                                                end
                                            end
                                    ...
                                    for aX, aY in pairs(workspace.Enemies:GetChildren()) do
                                        if (aY:FindFirstChild('HumanoidRootPart').Position - CFrame.new(- 22695.7012, 5270.93652, 3814.42847, 0.11794927, 3.32185834e-8, 0.99301964, - 8.73070718E-8, 1, - 2.30819008E-8, - 0.99301964, - 8.3975138E-8, 0.11794927).Position).Magnitude <= 300 then
                                            if aY:FindFirstChild('HumanoidRootPart') and aY:FindFirstChild('Humanoid') and aY:FindFirstChild('Humanoid').Health > 0 then
                                                repeat
                                                    wait()
                                                    m.Kill(aY, _G.CDK_TS)
                                                until not _G.CDK_TS or aY.Humanoid.Health <= 0 or not aY.Parent
                                            end
                                        end
                                    end
                                end
...
Other:AddSection("Bartillo")

Other:AddToggle("Bartillo", {
...
Other:AddSection("Observation")

Other:AddToggle("V2Obs", {
...
Raids:AddToggle("CompleteRaid", {
    ...
    Description = "Kill Mobs And Go Next Island",
    Default = false,
    Callback = function(Value)
        _G.Raiding = Value
    end
